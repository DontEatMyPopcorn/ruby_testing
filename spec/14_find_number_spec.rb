...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        expect(game_guessing.make_guess).to be(((game_guessing.min + game_guessing.max) / 2).floor)
      end
    end
  ...
  describe '#game_over?' do
    let(:correct_guess) { double('correct_guess') }
    subject(:game_guessing_3) { described_class.new(0, 9, correct_guess, 3) }
    ...
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        allow(correct_guess).to receive(:value).and_return(3)

        expect(game_guessing_3.game_over?).to be(true)
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        allow(correct_guess).to receive(:value).and_return(9)

        expect(game_guessing_3.game_over?).to be(false)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        ...
        expect(low_guess_game.min).to be(5)
      end

      it 'does not update max' do
        low_guess_game.update_range
        expect(low_guess_game.max).to be(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        ...
        expect(high_guess_game.min).to be(0)
      end

      it 'updates max to 8' do
        high_guess_game.update_range
        expect(high_guess_game.max).to be(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(edge_game.min).to be(8)
      end

      it 'does not update max' do
        edge_game.update_range
        expect(edge_game.max).to be(8)
      end
    end
  end
end
